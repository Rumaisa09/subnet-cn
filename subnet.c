...
printf("\n enter the number of hosts in the subnet:");
scanf("%d",&numHosts);
printf("\n enter the adjacency matrix(0 for no connection,1 for connection):\n");
for(i=0;i<numHosts;i++){
for(j=0;j<numHosts;j++){
scanf("%d",&adjMatrix[i][j]);
}
}
printf("\n enter the starting host for the broadcast(0 to %d):",numHosts-1);
scanf("%d",&startHost);
